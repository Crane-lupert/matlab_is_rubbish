...
df_class0 = df[df["fraud"] == 0].sample(n=200, random_state=90)
df_class1 = df[df["fraud"] == 1].sample(n=200, random_state=90)
...
# Train Set Fraud ratio:
print("Train Set Fraud ratio:\n", y_train.value_counts(), "\nratio:\n", y_train.value_counts(normalize=True))
print("Test Set Fraud ratio:\n", y_test.value_counts(), "\nratio:\n", y_test.value_counts(normalize=True))
...
def disabled_slider(label, min_val, max_val, value, step):
    '''
    Reference from GPT Code : to make certain input tag into invalid tag.
    '''
    html_str = f'''
...
app_ui = ui.page_fluid(
    ...
    # slider in fuction - to make it dynamic
    ...
    # conditional slider : DT - Max Depth, RF - Number of Estimators
    ui.output_ui("conditional_sliders"),
    # dynamic x axis choice
...
def server(input, output, session):
    ...
    # Table for confusion matrix and metrics.
    def build_confusion_metrics_table(y_true, y_pred, y_prob):
        '''
        Reference from GPT Code : to prettify the confusion matrix and metrics.
        '''
    ...
    # TSNE for PCA method for visualization. "src" col for keep test-train intact.
    def train_model(model_type, split, dt_ccp_alpha, rf_base_factor, max_depth, n_estimators):
        if split:
            ...
            # src col indicates the src of data: train or test, which prevents data mixing.
            X_train_full["src"] = "train"
            X_test_full["src"] = "test"
            combined = pd.concat([X_train_full, X_test_full], axis=0)
            tsne_result = TSNE(n_components=2, random_state=90).fit_transform(combined.drop(columns="src"))
            X_train_vis = pd.DataFrame(tsne_result[combined["src"]=="train"], columns=['TSNE1', 'TSNE2']).reset_index(drop=True)
            X_test_vis = pd.DataFrame(tsne_result[combined["src"]=="test"], columns=['TSNE1', 'TSNE2']).reset_index(drop=True)
        else:
            ...
            tsne_result = TSNE(n_components=2, random_state=90).fit_transform(X_train_full)
        ...
        if model_type == "dt":
            ...
            if not max_depth:
                print(f"Error on dragging max_depth slider: {max_depth}")
            ...
            if not n_estimators:
                print(f"Error on dragging n_estimators slider: {n_estimators}")
            ...
            model = RandomForestClassifier(n_estimators=n_estimators_val,
                                           ...
                                           random_state=90, oob_score=True)
        X_train_full.drop(columns="src", errors="ignore", inplace=True)
        X_test_full.drop(columns="src", errors="ignore", inplace=True)
        X_train_vis.drop(columns="src", errors="ignore", inplace=True)
        X_test_vis.drop(columns="src", errors="ignore", inplace=True)
    ...
    def plot_decision_boundary(model, X_origin, X_vis):
        '''
        Reference from GPT Code : to plot decision boundary and data scatter plot.
        '''
        Z_pred = model.predict(X_origin)
        padding = 0.5
        x_min, x_max = X_vis['TSNE1'].min() - padding, X_vis['TSNE1'].max() + padding
        y_min, y_max = X_vis['TSNE2'].min() - padding, X_vis['TSNE2'].max() + padding
        h = 200
        xx, yy = np.meshgrid(np.linspace(x_min, x_max, h), np.linspace(y_min, y_max, h))
        background_model = KNeighborsClassifier(n_neighbors=1).fit(X_vis, Z_pred)
        Background = background_model.predict(np.c_[xx.ravel(), yy.ravel()]).reshape((h, h))
